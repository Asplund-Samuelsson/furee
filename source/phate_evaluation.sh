...
split -d -l 64 results/evotuned/fbpase/validation_sequences.txt \
...
ls intermediate/validation_batches/batch_* | while read Infile; do
  ...
  source/get_representations.py -p results/evotuned/fbpase/iter_final \
  $Infile $Outfile
done
...
# Perform NMDS analysis of validation sequence representations
source/nmds_evaluation.R
